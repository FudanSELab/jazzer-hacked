...
public final class StructuredMutatorMazeFuzzer {
  ...
  private static void executeCommands(List<Command> commands, Consumer3<Byte, Byte, Boolean> callback) {
    byte x = 0;
    byte y = 0;
  ...
  private static char[][] parseMaze() {
    return Arrays.stream(StructuredMutatorMazeFuzzer.MAZE_STRING).map(String::toCharArray)
        .toArray(char[][]::new);
  }
